    ...
    <div className="space-y-6">
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 text-xs font-medium">{myClasses.length} {t('teacher.dashboard.myClasses').toLowerCase()}</span>
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 text-xs font-medium">{totalStudents} {t('nav.students').toLowerCase()}</span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <button onClick={() => navigate('/teacher/students')} className="card p-5 flex flex-col gap-3 hover:scale-[1.03] hover:shadow-md transition-all duration-200 text-start">
          ...
          <div><p className="text-2xl font-bold text-[var(--color-text)]">{totalStudents}</p><p className="text-xs text-[var(--color-text-muted)] mt-0.5">{t('teacher.dashboard.totalStudents')}</p></div>
        </button>
        <div className="card p-5 flex flex-col gap-3">
          ...
          <div><p className="text-2xl font-bold text-[var(--color-text)]">{totalSessions}</p><p className="text-xs text-[var(--color-text-muted)] mt-0.5">{t('teacher.dashboard.sessions')}/wk</p></div>
        </div>
        <div className="card p-5 flex flex-col gap-3 col-span-2 sm:col-span-1">
          ...
          <div><p className="text-2xl font-bold text-[var(--color-text)]">{todayCount}</p><p className="text-xs text-[var(--color-text-muted)] mt-0.5">{t('admin.dashboard.todayClasses')}</p></div>
        </div>
      </div>
        ...
        <div key={cls.id} className="card overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-blue-50 to-blue-100/50 dark:from-blue-900/20 dark:to-blue-900/10 border-b border-[var(--color-border)]">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-blue-600 flex items-center justify-center text-white font-bold text-sm">{cls.name.charAt(0)}</div>
              <div>
            ...
            <span className="badge bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">{cls.students.length} {t('nav.students').toLowerCase()}</span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--color-border)]">
                  {DAYS.map(d => (
                    <th key={d} className={`px-3 py-2.5 text-center text-xs font-semibold whitespace-nowrap ${d === todayDay ? 'text-brand-green-600 dark:text-brand-green-400' : 'text-[var(--color-text-muted)]'}`}>{t(`day.${d}`)}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  {DAYS.map(d => {
                    ...
                    return (
                      <td key={d} className={`px-3 py-4 text-center transition-colors ${isToday ? 'bg-brand-green-50 dark:bg-brand-green-900/20' : ''}`}>
                        {session ? (
                          session.timeType === 'prayer' ? <span className="text-xs text-brand-gold-600 dark:text-brand-gold-400 font-medium">{t(`prayer.${session.prayerRef}`)}</span> : (
                            <div><span className="text-xs text-brand-green-600 dark:text-brand-green-400 font-semibold block">{session.startTime}</span><span className="text-xs text-[var(--color-text-muted)]">{session.endTime}</span></div>
                          )
